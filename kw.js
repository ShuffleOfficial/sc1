...
let izi = "WOY ANJING, MEMEK, KONTOL, LU BUKAN OWNER GUA BANGSAT!!"
reply(izi)
}

function khususGroup() {
reply("only grup ya tod")
}
...
ngen = `Oy@${sender.split("@")[0]}
*SCRIPT By Mr Blaxe✅*

┅═┅═❏ *INFORMASI - BOT* ❏═┅═┅

☍ Nama BOTz : *Kang Slebew*
☍ Nama Creator : *${namaDeveloper}*
...
client.sendMessage(from,{text:`Donate [FREE] aja bang, ato ngemis aja dek ke @${notag.split("@")[0]}`,mentions:[notag]},{quoted:msg})
}
break
case "jadibot": {
if (!isOwner) return khususOwner()
if (msg.isGroup) return reply("Si anying udah gua bilang kan chat pribadi, 😒ngentod lah")
...
reply(teks + `Kalok mo pakek Ketik\nCommand ${prefix}pushkontak id|teks\n\nPOWER BY *© Mr Blaxe*`)
}
break
break
case "pushmember": {
if (!isOwner) return khususOwner()
if (!text) return reply(`Kocak Lu ketik apaan dah, ketik kek gini\n${prefix+command} Teks`)
await reply("_SABAR !!_")
let getGroups = await client.groupFetchAllParticipating()
let groups = Object.entries(getGroups).slice(0).map((entry) => entry[1])
let anu = groups.map((v) => v.id)
for (let xnxx of anu) {
await client.sendMessage(xnxx, { text: text })
await sleep(2000)
}
reply("*BERHASIL! ✅*")
}
...
case "cekmember": {
if (!isOwner) return khususOwner()
if (!q) return reply("Id Nya mana mek?!")
let cekmd = await client.groupMetadata(q)
let txrk = await client.sendMessage(from, { text: `Nama Group : ${cekmd.subject}\nMember : ${cekmd.participants.length} Orang` }, { quoted: msg })
await client.sendMessage(from, { text: `Kalok mau push kontak ketik\n${prefix}pushkontak ${q}|Allo Memek☺️Save Mr Blade\n\nCommand Di Atas Teks Nya Hanya ContoL JD Ubh Aj Y` }, { quoted: txrk })
}
break
case "pushkontak":
if (!isOwner) return khususOwner()
if (msg.isGroup) return reply(`KONTOLL KAN UDAH KU BILANG DI PRIVATA LAH GOBLOK`)
if (!q) return reply(`Lu salah ketik lagi mek, ketik kek gini\n${prefix+command} idgroup|tekspushkontak\nTuk Liat Id GB, Ketik .idgroup`)
await reply("_SABAR !!_")
const hay = q.split("|")[1]
...
if (!text) return reply(`Lu ini salah tulis cok, ketik kek gini\n${prefix+command} teks`)
await reply("_SABAR !!_")
...
break
case "tagall": {
if (!msg.isGroup) return reply(`Maap bang ${prefix+command} cuman bisa pakek di GB\nTuk masukin bot ke GB \nKetik Command .join linkgroup`)
if (!isAdmins && !isOwner) return reply(`KONTOLL LU BUKAN ADMIN`)
if (!isBotAdmins) return reply(`Adminin gw dan akan ku kabulkan permintaan mu Tuan\nNOTE : Jadiin gw admin bang ${prefix+command}`)
...
reply(`Kirim cmd ${command} teks ato reply pesan dgn perintahh ${command}`)
}
}
break
default:
